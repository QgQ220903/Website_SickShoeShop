...
class AdminOrderModel extends Database
{
  ...
  function insert($POST)
  {
    ...
    $query = "INSERT INTO `order` (`date`, user_id, customer_name, customer_phone, order_total, payment_method, `order_status`) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $this->conn->prepare($query);
    $stmt->execute([$currentDate, $user_id, $customerName, $customerPhone, $total, "Thanh toán tại quầy", 1]);
  ...
  {
    $query = "SELECT * FROM `order` ORDER BY `date` DESC LIMIT 1";
    $stmt = $this->conn->prepare($query);
    $stmt->execute();
    ...
    <table id='displayDataTable' class='table text-start align-middle table-bordered table-hover mb-0'>
      <thead>
        <tr class='text-dark'>
          ...
          <th scope='col' onclick='sortCol(\"o.date\")'>Ngày Tạo</th>
  ...
  function getDetail($POST)
  {
    ...
    <div class='modal-body'>
      ...
      <p class='mb-1'>Phương thức thanh toán: Thanh toán tại quầy</p>
    ";
